...
NamesAndTypesList IRowSchemaReader::readSchema()
{
            ...
            else if (data_types[i]->getName() != new_data_types[i]->getName())
            {
                if (default_type)
                    ...
                    throw Exception(
                        ...
                        "Automatically defined type for {} column differs in different rows", i);
            }
        }
    }
    ...
    for (size_t i = 0; i != data_types.size(); ++i)
    {
        /// Check that we could determine the type of all columns.
        if (!data_types[i])
        {
            if (!default_type)
                throw Exception(
        ...
        for (const auto & [name, new_type] : new_names_and_types)
        {
            ...
            else if (new_type && type->getName() != new_type->getName())
            {
                if (default_type)
                    ...
                    throw Exception(
                        ErrorCodes::CANNOT_EXTRACT_TABLE_STRUCTURE,
                        "Automatically defined type for column {} differs in different rows", name);
            }
        }
    }
