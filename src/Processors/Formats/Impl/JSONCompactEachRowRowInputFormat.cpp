...
void JSONCompactEachRowRowInputFormat::readPrefix()
{
    ...
    if (with_names)
    {
        ...
        JSONCompactEachRowRowNamesAndTypesReader names_and_types_reader;
        auto column_names = names_and_types_reader.readColumnNames(*in);
        for (const auto & column_name : column_names)
            addInputColumn(column_name);

        /// Type checking
        auto types = names_and_types_reader.readColumnDataTypes(*in);
...
Names JSONCompactEachRowRowNamesAndTypesReader::readColumnNames(ReadBuffer & in)
{
...
DataTypes JSONCompactEachRowRowNamesAndTypesReader::readColumnDataTypes(ReadBuffer & in)
{
    std::vector<String> type_names = readColumnNames(in);
...
void registerJSONCompactEachRowSchemaReader(FormatFactory & factory)
{
    ...
    {
        return std::make_shared<JSONCompactEachRowRowNamesAndTypesReader>();
    });
