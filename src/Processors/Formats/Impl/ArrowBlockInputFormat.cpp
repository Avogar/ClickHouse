...
ArrowSchemaReader::ArrowSchemaReader(ReadBuffer & in_, bool stream_) : ISchemaReader(in_), stream(stream_)
{
}
...
NamesAndTypesList ArrowSchemaReader::readSchema()
{
    ...
    if (stream)
        ...
        schema = createFileReader(in)->schema();
...
void registerArrowSchemaReader(FormatFactory & factory)
{
    factory.registerSchemaReader(
        ...
        {
            return std::make_shared<ArrowSchemaReader>(buf, false);
        });
    factory.registerSchemaReader(
        "ArrowStream",
        [](ReadBuffer & buf, const FormatSettings &)
        {
            return std::make_shared<ArrowSchemaReader>(buf, true);
        });}
}
#else
