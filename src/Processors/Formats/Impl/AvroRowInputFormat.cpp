...
{
    factory.registerSchemaReader("Avro", [](ReadBuffer & buf, const FormatSettings & format_settings)
    {
           return std::make_shared<AvroSchemaReader>(buf, false, format_settings);
    });

    factory.registerSchemaReader("AvroConfluent", [](ReadBuffer & buf, const FormatSettings & format_settings)
    {
        return std::make_shared<AvroSchemaReader>(buf, true, format_settings);
    });
