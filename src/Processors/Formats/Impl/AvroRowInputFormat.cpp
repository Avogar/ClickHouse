...
AvroSchemaReader::AvroSchemaReader(bool confluent_, const FormatSettings & format_settings_)
    : confluent(confluent_), format_settings(format_settings_)
{
}
...
{
    factory.registerSchemaReader("Avro", [](const FormatSettings & format_settings)
    {
           return std::make_shared<AvroSchemaReader>(false, format_settings);
    });

    factory.registerSchemaReader("AvroConfluent", [](const FormatSettings & format_settings)
    {
        return std::make_shared<AvroSchemaReader>(true, format_settings);
    });
