...
Names MsgPackNamesAndTypesReader::readColumnNames(ReadBuffer &)
{
    Names column_names;
...
DataTypes MsgPackNamesAndTypesReader::readColumnDataTypes(ReadBuffer & in)
{
    ...
    while (parsed_indexes.size() != number_of_columns && attempts <= max_depth_for_structure_determination)
    {
        for (size_t i = 0; i != number_of_columns; ++i)
        {
            ...
            if (!parsed_indexes.contains(i) && (data_types[i] = getDataType(object_handle.get())))
...
void registerMsgPackSchemaReader(FormatFactory & factory)
{
    ...
    {
        return std::make_shared<MsgPackNamesAndTypesReader>(settings);
    });
}
