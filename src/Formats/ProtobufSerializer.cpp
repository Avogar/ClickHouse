...
namespace
{
    ...
    NameAndTypePair getDataTypeFromField(const google::protobuf::FieldDescriptor * field_descriptor, bool allow_repeat = true)
    {
        if (allow_repeat && field_descriptor->is_repeated())
        {
            auto name_and_type = getDataTypeFromField(field_descriptor, false);
        ...
        switch (field_descriptor->type())
        {
            ...
            case FieldTypeId::TYPE_MESSAGE:
            {
                ...
                if (message_descriptor->field_count() == 1)
                {
                    ...
                    auto nested_name_and_type = getDataTypeFromField(nested_field_descriptor);
                ...
                else
                {
                    ...
                    {
                        auto nested_name_and_type = getDataTypeFromField(message_descriptor->field(i));
...
NamesAndTypesList protobufSchemaToCHSchema(const google::protobuf::Descriptor * message_descriptor)
{
    ...
    for (int i = 0; i != message_descriptor->field_count(); ++i)
        schema.push_back(getDataTypeFromField(message_descriptor->field(i)));
    return schema;
}
