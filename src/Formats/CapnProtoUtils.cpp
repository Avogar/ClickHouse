...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        schema = impl.parseDiskFile(schema_info.schemaPath(), schema_info.absoluteSchemaPath(), {});
#pragma GCC diagnostic pop
    }
    catch (const kj::Exception & e)
    {
        throw Exception(ErrorCodes::CANNOT_PARSE_CAPN_PROTO_SCHEMA, "Cannot parse CapnProto schema, error: {}", String(e.getDescription().cStr()));
    }
...
static String getCapnProtoFullTypeName(const capnp::Type & type)
{
    if (type.isStruct())
    {
        ...
        std::vector<String> non_union_type_names;
        for (auto nested_field : non_union_fields)
            non_union_type_names.push_back(getCapnProtoFullTypeName(nested_field.getType()));

        auto union_fields = struct_schema.getUnionFields();
        std::vector<String> union_type_names;
        for (auto nested_field : union_fields)
            union_type_names.push_back(getCapnProtoFullTypeName(nested_field.getType()));

        String union_name = "Union(" + boost::algorithm::join(union_type_names, ", ") + ")";
        /// Check if the struct is a named union.
        if (non_union_type_names.empty())
            return union_name;

        String type_name = "Struct(" + boost::algorithm::join(non_union_type_names, ",");
        /// Check if the struct contains unnamed union.
        if (!union_type_names.empty())
    ...
    if (!capnp_simple_type_names.contains(type.which()))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown CapnProto type");

    return capnp_simple_type_names.at(type.which());
}

template <typename Type>
static bool checkEnums(const capnp::EnumSchema & enum_schema, const EnumValues<Type> & enum_values, FormatSettings::EnumComparingMode mode, UInt64 max_value, String & error_message)
{
    ...
    if (mode == FormatSettings::EnumComparingMode::BY_NAMES)
    {
...
static bool checkTupleType(const capnp::Type & capnp_type, const DataTypePtr & data_type, FormatSettings::EnumComparingMode mode, String & error_message)
{
    ...
    for (const auto & name : tuple_data_type->getElementNames())
    {
        ...
        {
            if (!checkCapnProtoType(field, nested_types[tuple_data_type->getPositionByName(name)], mode, error_message))
                return false;
        }
        else
            return false;
...
static bool checkCapnProtoType(const capnp::Type & capnp_type, const DataTypePtr & data_type, FormatSettings::EnumComparingMode mode, String & error_message)
{
    switch (data_type->getTypeId())
    {
        ...
        case TypeIndex::Enum8:
            return checkEnums(capnp_type.asEnum(), *assert_cast<const EnumValues<Int8> *>(assert_cast<const DataTypeEnum<Int8> *>(data_type.get())), mode, INT8_MAX, error_message);
        case TypeIndex::Enum16:
            return checkEnums(capnp_type.asEnum(), *assert_cast<const EnumValues<Int16> *>(assert_cast<const DataTypeEnum<Int16> *>(data_type.get())), mode, INT16_MAX, error_message);
...
capnp::DynamicValue::Reader getReaderByColumnName(const capnp::DynamicStruct::Reader & struct_reader, const String & name)
{
    ...
    if (!struct_reader.has(field_name))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Capnproto struct doesn't contain field with name {}.", field_name);
    auto field_reader = struct_reader.get(field_name);
    if (nested_name.empty())
        return field_reader;

    if (field_reader.getType() != capnp::DynamicValue::STRUCT)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Field {} is not a struct.", field_name);
...
static capnp::StructSchema::Field getFieldByName(const capnp::StructSchema & schema, const String & name)
{
    ...
    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Capnproto schema doesn't contain field with name {}.", field_name);
}
...
void checkCapnProtoSchemaStructure(const capnp::StructSchema & schema, const Block & header, FormatSettings::EnumComparingMode mode)
{
    ...
    if (checkIfStructContainsUnnamedUnion(schema))
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Schema contains unnamed union that is not supported.");
