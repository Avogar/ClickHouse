...
/// filter it by mask, reduce and then expand by mask. If inverted is true, we will work with inverted mask.
MaskInfo maskedExecute(ColumnWithTypeAndName & column, const PaddedPODArray<UInt8> & mask, bool inverted = false, bool expand = true);
