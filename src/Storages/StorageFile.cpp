...
void StorageFile::setStorageMetadata(CommonArguments args)
{
    ...
    else if (args.columns.empty())
    {
        if (FormatFactory::instance().checkIfFormatHasSchemaReader(args.format_name))
        {
            ...
            try
            {
                auto schema_reader = FormatFactory::instance().getSchemaReader(args.format_name, *read_buf, args.getContext(), format_settings);
                schema = schema_reader->readSchema();
            }
            catch (const DB::Exception & e)
            {
                throw Exception(ErrorCodes::INCORRECT_DATA, "Cannot extract table structure from {} format file. Error: {}", args.format_name, e.message());
            }
            storage_metadata.setColumns(ColumnsDescription(schema));
        }
        else
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "{} file format doesn't support schema interface", args.format_name);
    }
    else
